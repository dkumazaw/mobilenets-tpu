...
class ImageNetTFExampleInput(object):
    ...
    Args:
        is_training (bool):    Denotes whether the input is for training
        use_bfloat16 (bool):   If True, use bfloat16 precision; else use float32.
        num_cores (int):       Number of TPU cores
        image_size (int):      Image size (both width and height).
        transpose_input(bool): Denotes whether to use the double transpose trick
    """
    ...
    def input_fn(self, params):
        ...
        if self.transpose_input:
            dataset = dataset.map(
                lambda images, labels: (tf.transpose(
...
class ImageNetInput(ImageNetTFExampleInput):
    ...
    def __init__(self,
        ...
        Args:
          is_training (bool):       Whether the input is for training
          use_bfloat16 (bool):      If True, use bfloat16 precision; else use float32.
          transpose_input (bool):   Whether to use the double transpose trick
          data_dir (str):           String for the directory of the training and validation data;
                                        if 'null' (the literal string 'null') or implicitly False
                                        then construct a null pipeline, consisting of empty images
                                        and blank labels.
          image_size (int)          Image size (both width and height).
          num_parallel_calls (int): Concurrency level to use when reading data from disk.
          cache (bool):             If true, fill the dataset by repeating from its cache
        """
        super().__init__(
    ...
    def dataset_parser(self, value):
        ...
        return super().dataset_parser(value)
...
# --------------------------------------------------------------------------------
# NOTE: Below is kept for potential bigtable support in the future
# --------------------------------------------------------------------------------
